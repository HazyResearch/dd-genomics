...
CHARITE_SUP_RGX = (
  ...
  [
    ...
    '(disrupt|mutat|delet|duplicat|truncat|SNP).*(caus|responsible for)',
    '{{P}}.*secondary to.*{{G}}',
...
POS_RGX = (
  ...
  12,
  [
    '(disrupt|expan.*repeat|mutat|delet|duplicat|truncat|SNP|polymorphism).*{{G}}.*cause.*{{P}}',
    ...
    #r'{{P}} results from.*{{G}}',
    #r'mutations in {{G}}.*(result in|lead to).*{{P}}',
    #r'causes of {{P}}.*include.*{{G}}',
    #r'{{P}}.*caused by mutations in.*{{G}}',
    #r'{{P}}.*is a result of mutations in.*{{G}}',
    #r'{{G}}.*MIM # \d+.*{{P}}',
    #r'{{G}}.*mutation causing.*{{P}}'
  ])
