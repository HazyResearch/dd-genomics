...
Label = namedtuple('Label', 'relation_id, rule_id, label')
...
def apply_rules(relation_id, rule_triple, f, boolean_only=True):
  """
  Takes as input a triple containing (rule_label, default_weight, rules)
  Where rules is a list of rules which are each either a pattern or a (pattern, weight) tuple/list
  Applies f to the pattern
  If true, yields a Label object, with the rule's weight or else the rule_triple's default weight
  If boolean_only=True, then just takes sign of weight
  """
  rule_id_base, w, rules = rule_triple
