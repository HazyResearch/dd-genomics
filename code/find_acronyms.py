...
def extract(sentence):
    ...
    if len(sentence.words) > 0 and sentence.words[0].word.casefold() == "abbreviations":
        index = [x.word for x in sentence.words].index(":") + 1
        ...
        for word in sentence.words:
            if word.word.casefold() not in merged_genes_dict:
                continue
            acronym = None
            # Look for definition only if this word has length at least 2 and
            # is all capitals and it comes between "(" and ")" or "(" and ";"
            # or "(" and ","
            ...
            and sentence.words[word.in_sent_idx + 1].word in [")", ";" ","]:
                ...
                while start_idx + window_size - 1 < word_idx:
                    ...
                    if is_definition and definition.casefold() not in merged_genes_dict:
                        acronym = dict()
