...
cat manual/charite_all_sources_gene_to_disease_to_pheno.tsv | cut -f 1,3 | sort | uniq >> data/charite_supervision_ensgene.tsv
join -1 2 -2 2 -t $'\t' \
  ...
  <(cat manual/phenotypic_series_to_omim.tsv | 
    sort | uniq | sort -k2,2) |
  cut -f 2,3 | sort | uniq >> data/charite_supervision_ensgene.tsv
join -1 2 -2 1 -t $'\t' \
