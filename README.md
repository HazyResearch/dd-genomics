...
1. Make sure environment variables are set correctly- see `env.sh`
   ...
   NOTE: If using plpy extractors [we are not as of 4/7/15...], add `export DD_GENOMICS_HOME=...` to the `~/.bashrc` where the DB runs. 
...
2. Create the database to be used if necessary, then if the pre-processed raw input data has been loaded (the `sentences` and/or `sentences_input` tables), just run:

		For Greenplum: `./util/create_schema.sh`
		For Postgres: `./util/create_schema.sh pg`

  Note that this will drop any existing output data, e.g. data generated by dd-genomics or deepdive.  If the raw input data hasn't been loaded, run: 
