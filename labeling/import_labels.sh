...
psql -U $DDUSER -p 6432 -d genomics_$DDUSER -c 'CREATE TABLE genepheno_causation_labels(relation_id VARCHAR, is_correct VARCHAR, labeler VARCHAR)'
...
psql -U $DDUSER -p 6432 -d genomics_$DDUSER -c 'CREATE TABLE genepheno_association_labels(relation_id VARCHAR, is_correct VARCHAR, labeler VARCHAR)'  
...
psql -U $DDUSER -p 6432 -d genomics_$DDUSER -c 'CREATE TABLE gene_labels(mention_id VARCHAR, is_correct VARCHAR, labeler VARCHAR)'
...
psql -U $DDUSER -p 6432 -d genomics_$DDUSER -c 'CREATE TABLE pheno_labels(relation_id VARCHAR, is_correct VARCHAR, labeler VARCHAR)'
