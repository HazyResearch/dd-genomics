...
from
  ...
  join (SELECT * FROM pheno_mentions p WHERE p.entity = 'HP:0000252' OR p.entity = 'HP:0000708' OR p.entity = 'HP:0000707' AND (p.is_correct = 't' OR p.is_correct is null) LIMIT 1000) p
    on (si.doc_id = p.doc_id
        and si.section_id = p.section_id
        and si.sent_id = p.sent_id)
where
  (g.is_correct = 't' OR g.is_correct is null)
LIMIT 500
)
