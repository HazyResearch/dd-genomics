...
deepdive sql "COPY (SELECT gal.relation_id, gal.is_correct, gal.labeler, gi.expectation  FROM genepheno_causation_labels gal, genepheno_causation_inference_label_inference gi WHERE gal.relation_id = gi.relation_id AND gi.expectation >= $CONFIDENCE AND gal.is_correct = 't') TO STDOUT WITH NULL AS ''" > $DIR_NAME/TP.tsv
...
# STORE holdout set
deepdive sql "COPY (SELECT * FROM genepheno_causation_labels gal) TO STDOUT WITH NULL AS ''" > $DIR_NAME/holdout_set.tsv
