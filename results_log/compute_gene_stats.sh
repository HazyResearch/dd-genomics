...
deepdive sql "COPY (SELECT gl.mention_id, gl.is_correct, gl.labeler FROM gene_labels gl, gene_mentions_filtered gm WHERE gl.mention_id=gm.mention_id) TO STDOUT WITH NULL AS ''" > gene_labels.tsv
...
deepdive sql "COPY (SELECT gl.mention_id, gl.is_correct, gl.labeler, gi.expectation FROM gene_labels gl, gene_mentions_filtered_inference_label_inference gi WHERE gl.mention_id = gi.mention_id AND gi.expectation >= $CONFIDENCE AND gl.is_correct = 't') TO STDOUT WITH NULL AS ''" > $DIR_NAME/TP.tsv
...
gi.expectation >= $CONFIDENCE AND gl.is_correct = 'f') TO STDOUT WITH NULL AS ''" > $DIR_NAME/FP.tsv

# FN 
deepdive sql "COPY (SELECT gl.mention_id, gl.is_correct, gl.labeler, gi.expectation FROM gene_labels gl, gene_mentions_filtered_inference_label_inference gi WHERE gl.mention_id = gi.mention_id AND 
gi.expectation < $CONFIDENCE AND gl.is_correct = 't') TO STDOUT WITH NULL AS ''" > $DIR_NAME/FN.tsv

# STORE holdout set
deepdive sql "COPY (SELECT * FROM gene_labels) TO STDOUT WITH NULL AS ''" > $DIR_NAME/holdout_set.tsv
