...
deepdive sql "COPY (SELECT gal.relation_id, gal.is_correct, gal.labeler FROM genepheno_association_labels gal, genepheno_pairs gp WHERE (gp.gene_mention_id || ('_' || gp.pheno_mention_id)) = gal.relation_id AND gal.version=$VERSION) TO STDOUT WITH NULL AS ''" > genepheno_association_labels.tsv

# extract all predictions from the genepheno_association table
deepdive sql "COPY (SELECT gi.relation_id, gi.expectation FROM genepheno_association_inference_label_inference gi, genepheno_association_labels gal WHERE gal.relation_id = gi.relation_id AND gal.version=$VERSION) TO STDOUT WITH NULL AS ''" > genepheno_association_predictions.tsv
...
deepdive sql "COPY (SELECT gal.relation_id, gal.is_correct, gal.labeler, gi.expectation  FROM genepheno_association_labels gal, genepheno_association_inference_label_inference gi WHERE gal.relation_id = gi.relation_id AND gi.expectation >= $CONFIDENCE AND gal.is_correct = 't' AND gal.version=$VERSION) TO STDOUT WITH NULL AS ''" > $DIR_NAME/TP.tsv
...
# STORE holdout set
deepdive sql "COPY (SELECT * FROM genepheno_association_labels gal WHERE gal.version=$VERSION) TO STDOUT WITH NULL AS ''" > $DIR_NAME/holdout_set.tsv
