...
deepdive sql "COPY (SELECT gal.relation_id, gal.is_correct, gal.labeler FROM genepheno_causation_precision_labels gal, genepheno_pairs gp WHERE (gp.gene_mention_id || ('_' || gp.pheno_mention_id)) = gal.relation_id AND gal.version = $VERSION) TO STDOUT WITH NULL AS ''" > genepheno_causation_labels.tsv

# extract all predictions from the genepheno_causation table
deepdive sql "COPY (SELECT gi.relation_id, gi.expectation FROM genepheno_causation_inference_label_inference gi, genepheno_causation_precision_labels gal WHERE gal.relation_id = gi.relation_id AND gal.version = $VERSION) TO STDOUT WITH NULL AS ''" > genepheno_causation_predictions.tsv
# launch python script that computes precision and recall
$GDD_HOME/results_log/compute_stats_helper.py genepheno_causation_labels.tsv genepheno_causation_predictions.tsv $CONFIDENCE > $GDD_HOME/results_log/latest_stats_causation_precision.tsv
...
# Store logs
DIR_NAME=$GDD_HOME/results_log/$DDUSER/causation_precision-`date +'%m-%d-%Y-%H-%M-%S'`
mkdir -p $DIR_NAME

# Store stats in log
cat $GDD_HOME/results_log/latest_stats_causation_precision.tsv > $DIR_NAME/stats_causation_precision.tsv
