...
false_pos=`psql -X --set ON_ERROR_STOP=1 -d ${DBNAME} -c 'COPY (SELECT COUNT(*) FROM genepheno_facts_is_correct_inference WHERE expectation > 0.9 AND is_correct = false) TO STDOUT'`
